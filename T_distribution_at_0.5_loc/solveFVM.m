...
for i = 1:size(M, 1)
    for j = 1:size(M, 2)
        ...
        [A(index(i,j), :), B] =  stamp(i, j, X, Y, B, solid_type, Tw_solid_flow_s, Tw_west , Q_south, Q_east);     
    end
end
...
if strcmp(solid_type,"solid1")
    k_solid = 0.01;              % Thermal conductivity of nickel alloys = 300W/mK
    Q_out1 = abs(((k_solid.*(T(1,:)- T(2,:))./(Y(1,:)-Y(2,:)))));
    Q_out2 = abs(((k_solid.*(T(:,1)- T(:,2))./(X(:,1)-X(:,2)))));
end 
